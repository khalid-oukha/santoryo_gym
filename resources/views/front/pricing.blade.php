        ...
        <div class="container px-4 mt-4">
            <div class="flex flex-wrap justify-center ">
                @foreach ($offers as $offer)
                    <div class="w-full md:w-1/4 mx-4 px-4 mb-8">
                        <div
                            class="w-full px-4 py-2 transition-colors duration-300 transform rounded-lg  md:w-96 bg-dark-400 dark:bg-gray-800">

                            <form action="{{ route('pay.order') }}" method="post">
                                @csrf
                                <input type="hidden" name="offer_id" value="{{ $offer->id }}">
                                <div class="text-center">
                                    <p class="text-2xl font-semibold text-gray-200 dark:text-gray-100">{{ $offer->title }}
                                    </p>
                                    ...
                                    <h4 class="mt-2 text-4xl font-semibold text-gray-200 dark:text-gray-100">
                                        {{ $offer->price * 10 }}
                                        DH</h4>
                                    <p class="mt-4 text-gray-300 dark:text-gray-300">/per {{ $offer->months_valid }} month
                                    </p>
                                </div>
